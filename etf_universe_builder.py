...
def _classify(df):
    def classify(name):
        ...
        if any(kw in u for kw in ['금광','GOLD MINERS','GOLDMINERS','금채굴']): return '원자재','금광기업',''
        ...
        if any(kw in u for kw in ['희토류','RARE EARTH']): return '원자재','희토류',''
        ...
        if any(kw in u for kw in ['UAE','두바이','아부다비']): return '해외주식','UAE',''
        if any(kw in u for kw in ['튀르키예','터키','TURKEY']): return '해외주식','튀르키예',''
        ...
        if any(kw in u for kw in ['HBM','고대역폭메모리']): return '섹터/테마','HBM/메모리',''
        if any(kw in u for kw in ['반도체','팹리스','파운드리']): return '섹터/테마','반도체',''
        if any(kw in u for kw in ['2차전지','배터리','리튬','양극재','음극재']): return '섹터/테마','2차전지/배터리',''
        if any(kw in u for kw in ['양자컴퓨팅','양자','QUANTUM']): return '섹터/테마','양자컴퓨팅',''
        if any(kw in u for kw in ['온디바이스','ON-DEVICE','온디바이스AI']): return '섹터/테마','온디바이스AI',''
        if any(kw in u for kw in ['AI인프라','AI INFRA','AI 인프라']): return '섹터/테마','AI인프라',''
        if any(kw in u for kw in ['AI','인공지능','AI반도체','AI밸류체인']): return '섹터/테마','AI',''
        if any(kw in u for kw in ['데이터센터','DATACENTER','DATA CENTER','IDC']): return '섹터/테마','데이터센터',''
        ...
        if any(kw in u for kw in ['휴머노이드','HUMANOID']): return '섹터/테마','휴머노이드',''
        ...
        if any(kw in u for kw in ['필수소비재','음식료','식품','F&B','K-푸드','K푸드','K-FOOD','K푸드']): return '섹터/테마','필수소비재/식품',''
        if any(kw in u for kw in ['경기소비재','럭셔리','명품','LUXURY','화장품','뷰티']): return '섹터/테마','경기소비재/뷰티',''
        ...
        # 메타버스/XR — 짧은 라틴 키워드(AR/VR/XR)는 단어경계 필요 (KBSTAR의 AR 오매칭 방지)
        if '메타버스' in u or re.search(r'(?:^|\W)(?:XR|VR|AR)(?:$|\W)', u):
            return '섹터/테마','메타버스/XR',''
        if any(kw in u for kw in ['블록체인','디지털자산','비트코인','가상자산','크립토']): return '섹터/테마','블록체인/가상자산',''
        if any(kw in u for kw in ['핀테크','FINTECH']): return '섹터/테마','핀테크',''
        if any(kw in u for kw in ['결제','페이먼트','PAYMENT','PAYMENTS','페이먼츠']): return '섹터/테마','결제/페이먼트',''
        if any(kw in u for kw in ['플랫폼','인터넷','이커머스','커머스']): return '섹터/테마','플랫폼/인터넷',''
        if any(kw in u for kw in ['IT','테크','기술','ICT']): return '섹터/테마','IT/테크',''
        # 최근 테마 추가
        if any(kw in u for kw in ['IPO','신규상장','SPAC']): return '섹터/테마','IPO/신규상장',''
        if any(kw in u for kw in ['혁신','INNOVATION','이노베이션','파괴적']): return '섹터/테마','혁신/이노베이션',''
...
def _sub(u, country):
    if country == '미국':
        if any(kw in u for kw in ['M7','MAGNIFICENT','매그니피센트','TOP7','빅테크7']): return '미국M7/빅테크'
        ...
        if any(kw in u for kw in ['러셀','RUSSELL','RUSSELL2000']): return '러셀2000/소형'
        if any(kw in u for kw in ['HBM','메모리']): return '미국HBM/메모리'
        if any(kw in u for kw in ['반도체','필라델피아']): return '미국반도체'
        if any(kw in u for kw in ['양자','QUANTUM']): return '미국양자컴퓨팅'
        if any(kw in u for kw in ['휴머노이드','HUMANOID']): return '미국휴머노이드'
        if any(kw in u for kw in ['AI인프라','AI 인프라','AI INFRA']): return '미국AI인프라'
        if any(kw in u for kw in ['데이터센터','DATACENTER','DATA CENTER']): return '미국데이터센터'
        if any(kw in u for kw in ['빅테크','테크','FANG','TECH']): return '미국테크'
        if any(kw in u for kw in ['배당','DIVIDEND','SCHD']): return '미국배당'
        ...
        if any(kw in u for kw in ['유틸리티','전력']): return '미국유틸리티'
        if any(kw in u for kw in ['혁신','INNOVATION','이노베이션']): return '미국혁신'
        return '미국(기타)'
    if country == '일본':
