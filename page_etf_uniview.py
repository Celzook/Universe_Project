...
ETF Uniview Page
================
- AP (Actual Portfolio) 업로드 + 초과성과 분석
- 저장된 MP 의 편입일 기준 forward 성과
- 🔥 Hot Sectors + Live MP (Core-Satellite)
- 🔄 MP 룰 자동 시뮬레이션 (Phase 1)
- 전체 유니버스 C_score 랭킹 보드

* 개별 ETF 차트 매트릭스는 사이드바의 「📊 차트 매트릭스」 페이지로 분리되었음.
"""
import hashlib
import json
import re
...
from etf_scoring import score_all_kr_etfs
...
def page_etf_uniview():
    ...
    st.caption(
        f"📊 개별 ETF 차트 매트릭스는 사이드바 메뉴 **「📊 차트 매트릭스」** 로 이동했습니다. "
        f"이 페이지는 AP/MP 분석 + 전체 유니버스 스코어 랭킹에 집중."
    )
