...
# app.py 맨 아래 탭 부분에 임시 추가
with tab_진단:  # 또는 기존 탭 구조에 맞게 추가
    if st.button("구성종목 수집 진단 실행"):
        import requests, io, pandas as pd
        TICKER = "069500"
        
        # 테스트 1: 모바일 API
        st.subheader("1. 네이버 모바일 API")
        try:
            r = requests.get(
                f"https://m.stock.naver.com/api/stock/{TICKER}/etf/portfolio",
            ...
            if r.status_code == 200:
                data = r.json()
                st.json(data if isinstance(data, dict) 
                        else {"list_len": len(data), "first": data[0] if data else {}})
            else:
                st.write(r.text[:200])
        except Exception as e:
            st.error(str(e))
        
        # 테스트 2: etfinfo.naver HTML
        st.subheader("2. etfinfo.naver (CU당 구성종목)")
        try:
            r2 = requests.get(
                f"https://finance.naver.com/item/etfinfo.naver?code={TICKER}",
                headers={'User-Agent': 'Mozilla/5.0'}, timeout=10
            )
            st.write(f"Status: {r2.status_code}, 길이: {len(r2.content)}")
            html = r2.content.decode('euc-kr', errors='ignore')
            st.write("키워드 존재:", {kw: (kw in html) 
                     for kw in ['구성종목','비중','종목명','<table']})
            
            # 구성종목 주변 텍스트
            idx = html.find('구성종목')
            if idx >= 0:
                st.code(html[idx:idx+500])
            
            tables = pd.read_html(io.StringIO(html))
            st.write(f"테이블 {len(tables)}개 발견")
            for i, t in enumerate(tables):
                st.write(f"[{i}] {t.shape} | {t.columns.tolist()[:5]}")
        except Exception as e:
            st.error(str(e))
